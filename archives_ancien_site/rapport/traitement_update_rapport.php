...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $updates = [
        ...
        "annee = $annee",
        "trimestre = $trimestre_sql"
    ];

    // Handle PDF Upload
    if (isset($_FILES["pdf_file"]) && $_FILES["pdf_file"]["error"] == 0) {
        $pdf_name = time() . "_" . basename($_FILES["pdf_file"]["name"]);
        $pdf_name = preg_replace('/[^a-zA-Z0-9_\.-]/', '_', $pdf_name); 
    ...
    $sql = "UPDATE rapports SET " . implode(", ", $updates) . " WHERE id = $id";
    
    if ($conn->query($sql) === TRUE) {
        header("Location: admin_rapports.php?message=Rapport mis à jour avec succès");
        exit();
    } else {
