...
if ($result_actualites && $result_actualites->num_rows > 0):
    while ($row = $result_actualites->fetch_assoc()):
        ...
        // Nettoyage du titre (décodage si déjà encodé dans la BDD pour éviter les &#039; apparents)
        $title = html_entity_decode($row['title'], ENT_QUOTES | ENT_HTML5, 'UTF-8');
        $display_title = (mb_strlen($title) > 60) ? mb_substr($title, 0, 60) . '...' : $title;
        // Re-sécurisation propre sans double encodage
        $display_title = htmlspecialchars($display_title, ENT_QUOTES, 'UTF-8');
?>
        <div class="group relative bg-white border border-gray-100 rounded-[2rem] overflow-hidden hover:border-[#ea750fff]/50 transition-all duration-500 shadow-md hover:shadow-xl fade-in-card">
            ...
            <div class="p-6 flex flex-col justify-between h-[calc(100%-14rem)]">
                <div>
                    <span class="text-gray-500 text-xs flex items-center gap-2 mb-3 cabin-sketch">
                        <i class="far fa-calendar-alt text-[#ea750fff]"></i>
                        <?= date('d M Y', strtotime($row['end_date'])) ?>
                    </span>
                    <h5 class="cabin-sketch text-[#0f277e] text-lg font-bold leading-tight mb-6 group-hover:text-[#ea750fff] transition-colors">
